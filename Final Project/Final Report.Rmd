...
```{r}
salary_prediction <- function(m, point, minutes, turn_over, assists, steals, rebounds){
  pre_new <- predict(m, data.frame(PTS = point, MP = minutes, TOV= turn_over, AST= assists, STL= steals, TRB= rebounds))
  msg <- paste( "Expected Salary: $", format(round(pre_new), big.mark = ","), sep = "")
  print(msg)
}
model <- lm(salary18_19~ PTS + MP + TOV + AST + STL + TRB, data =regression)
salary_prediction(model,Pope$PTS,Pope$MP,Pope$TOV,Pope$AST,Pope$STL,Pope$TRB)
salary_prediction(model,Bullock$PTS,Bullock$MP,Bullock$TOV,Bullock$AST,Bullock$STL,Bullock$TRB)
salary_prediction(model,Stephenson$PTS,Stephenson$MP,Stephenson$TOV,Stephenson$AST,Stephenson$STL,Stephenson$TRB)
```
So Salaries for Pope, Bullock and Stephenson for next season are $7,785,809, $7,126,513 and $5,286,590
